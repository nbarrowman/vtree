---
title: "Introduction to vtree 3.1.4"
...
As an example, if you call `svtree` without specifying any variables,
it uses as many discrete variables from the data frame as possible,
and it also sets `showvarinnode=TRUE` by default:

```{r, eval=FALSE}
svtree(FakeData)
```

A message is displayed indicating which variables have been included and excluded:

```{r, eval=FALSE}
--Discrete variables included: Group Severity Sex Age Male Category Event Ind1 Ind2
--Discrete variables excluded: Ind3 Ind4 Viral
Additional variables excluded: id Score Pre Post Post2 Time
```
