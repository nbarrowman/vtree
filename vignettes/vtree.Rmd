---
title: "Introduction to vtree 4.2.7"
...
For example, we might wish to display the mean `Score` for individuals in each node.
Here's how to do this, using the `summary` parameter:
...
`%freqpctlines%`|  same as `%freqpct%` but on separate lines
`%freq%`        |  frequency of values of a variable
`%freqlines%`   |  same as `%freq%` but on separate lines
...
The `%freqpct%` code allows you to summarize a discrete variable within nodes.
For example, an alternative to a variable tree for `Sex` and `Severity`
is a variable tree for `Sex` with a summaries for `Severity` in each node. 

```{r,eval=FALSE}
vtree(FakeData,"Sex",summary="Severity \n%freqpct%")
```

`r vtree(FakeData,"Sex",summary="Severity \n%freqpct%",
  width=450,height=150,pxwidth=1000,imageheight="3in")`

Because (by default) a summary is shown in each node,
this tree shows more information than a variable tree for `Sex` and `Severity`.
The display is also more compact, which can be convenient,
especially with large trees.
