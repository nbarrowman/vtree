---
title: "Introduction to vtree 4.4.0"
...
But `Ind*` would also match variables with names like `Ind`, `Index`, and `India1977`.


### prefix `any:` with suffix `*` or `#`

Sometimes a group of variables contain responses to 
a list of "check all that apply" options (i.e., checkboxes).
It is often useful to know if *any* of the options were selected (i.e. checked).

A specification like `any:Start#` (`any:Start*`) 
...
Values that do not correspond to the above are treated as missing values.
