---
title: "Introduction to vtree 2.0.1"
...
```{r, echo=FALSE}
v1 <- vtree(FakeData,"Severity Sex",showlegend=FALSE,horiz=FALSE,width=600,height=200,pngknit=FALSE)
...
```
```{r,echo=FALSE}
v2 <- vtree(FakeData,"Severity",width=250,height=250,pngknit=FALSE)
...
```
```{r, echo=FALSE}
v3 <- vtree(FakeData,"Severity Sex",horiz=FALSE,plain=TRUE,width=550,height=200,pngknit=FALSE)
...
```
```{r,echo=FALSE}
v4 <- vtree(FakeData,"Severity Sex",showlegend=TRUE,shownodelabels=FALSE,width=550,height=450,pngknit=FALSE)
...
v6 <- vtree(FakeData,"Severity Sex",prune=list(Severity=c("Mild","Moderate")),
  width=500,height=300,pngknit=FALSE)
...
v8 <- vtree(FakeData,"Severity Sex",horiz=FALSE,width=700,height=250,
  labelvar=c(Severity="Initial severity"),pngknit=FALSE)
...
v8a <- vtree(FakeData,"Group Sex",horiz=FALSE,labelnode=list(Sex=c(Male="M",Female="F")),
  width=700,height=250,pngknit=FALSE)
...
v8b <- vtree(FakeData,"Group Sex",horiz=FALSE,width=700,height=250,
  ...
  tlabelnode=list(
    ...
    c(Group="A",Sex="M",label="boy"),
    c(Group="B",Sex="F",label="woman"),
    c(Group="B",Sex="M",label="man")),pngknit=FALSE)
...
v9 <- vtree(FakeData,"Group Severity",horiz=FALSE,showvarnames=FALSE,
  text=list(Severity=c(Mild="\n*Excluding\nnew diagnoses*")),
  width=450,height=150,pngknit=FALSE)
...
v10 <- vtree(FakeData,"Severity",summary="Score \nmean score: %mean%",
  sameline=TRUE,horiz=FALSE,width=450,height=150,pngknit=FALSE)
...
v11 <- vtree(FakeData,"Severity",horiz=FALSE,showvarnames=FALSE,splitwidth=Inf,sameline=TRUE,
  summary=c(
    "Score \nScore: mean (SD) %mean% (%SD%)",
    "Pre \nPre: range %min%, %max%"),
  width=650,height=350,pngknit=FALSE)
...
v12 <- vtree(FakeData,"Severity",summary="Viral \nViral %npct%",horiz=FALSE,
  showvarnames=FALSE,sameline=TRUE,width=650,height=200,pngknit=FALSE)
...
v13x <- vtree(FakeData,"Severity",summary="Category=single \n%pct% single", 
  horiz=FALSE,sameline=TRUE,width=450,height=150,pngknit=FALSE)
...
v13a <-
  ...
  summary=c(
    "Viral \nviral: %npct%%leafonly%",
    "Feature1 , F1: %npct%%leafonly%",
    "Feature2 , F2: %npct%%leafonly%"),pngknit=FALSE)
...
v13b <- vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=150,varminwidth=c(Category=6),
  ...
  runsummary=list(
    function(x) TRUE,
    function(x) any(x$Viral,na.rm=TRUE),
    function(x) any(x$Viral,na.rm=TRUE)),pngknit=FALSE)
...
```
```{r, echo=FALSE}
v14 <- vtree(FakeData,"Severity Sex",width=650,height=650,pngknit=FALSE)
filepath <- grVizToPNG(v14,folder=PNGdir,width=800)
v15 <- vtree(FakeData,"Severity Sex",pattern=TRUE,width=650,height=650,pngknit=FALSE)
...
```
```{r, echo=FALSE}
v21 <- vtree(FakeData,"Ind1 Ind2 Ind3 Ind4",Venn=TRUE,width=650,height=650,pngknit=FALSE)
...
v23 <- vtree(FakeData,"Ind1 Ind2 Ind3 Ind4",Venn=TRUE,pattern=TRUE,
  palette=c(Ind1=1,Ind2=2,Ind3=3,Ind4=4),pngknit=FALSE)
...
v24 <- vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,
  width=700,height=370,pngknit=FALSE)
...
v26 <- vtree(FakeRCT,"eligible randomized group followup analyzed",plain=TRUE,width=230,height=500,
  keep=list(eligible="Eligible",randomized="Randomized",followup="Followed up"),
  horiz=FALSE,showvarnames=FALSE,title="Assessed for eligibility",pngknit=FALSE)
