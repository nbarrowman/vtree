---
title: "Introduction to vtree 4.1.1"
...
[examples using R datasets](#RdatasetExamples) that you can try.
...
There are parameters for specifying the color of (1) missing value nodes and
(2) the root node, described below.
Excluding those special nodes,
all the other nodes of the tree can be made the same color (say `palegreen`)
by specifying `fillcolor="palegreen"`.

By default, missing value nodes are colored white.
...
The color of the root node can be changed (say to `yellow`)
by specifying `rootfillcolor="yellow"`.

A simple color scheme together with well-spaced nodes is produced by specifying
`plain=TRUE`.
