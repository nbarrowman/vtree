---
title: "Introduction to vtree 4.6.2"
...
vtree produces variable trees and automatically labels them with counts,
...
on whether the variables have any missing values, as discussed below.
...
In comparison, when manual calculation and transcription are used to
populate this sort of diagram, mistakes are likely.
...
In this case, "Severity" is shown below the corresponding nodes.
...
The `lsplitwidth` parameter is used to control text wrapping in the legend.
...
To compare, here's what happens when `vp=FALSE`:
...
*This section shows how to display additional information in nodes.*
...
a specified variable within a subset of the data frame.
...
`r vtree(FakeData,"Severity",summary="Ind*",sameline=TRUE,horiz=FALSE,just="l",
  pxwidth=1100,imageheight="2.5in",margin=0.3)`
