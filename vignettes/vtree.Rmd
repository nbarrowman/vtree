...
(Feature I and Feature II) is recorded.
...
```{r}
set.seed(1234)
FakeData$FeatureI <- rbinom(nrow(FakeData),1,0.5)
FakeData$FeatureI[!FakeData$Viral | is.na(FakeData$Viral)] <- NA
FakeData$FeatureII <- rbinom(nrow(FakeData),1,0.5)
...
and number and percent of viruses with Feature I and with Feature II.

```{r,eval=FALSE}
vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=50,
  summary=c(
    "Viral viral: %npct% %leafonly%",
    "FeatureI \nFeature I: %npct% %leafonly%",
    "FeatureII \nFeature II: %npct% %leafonly%"))
```
```{r,echo=FALSE}
options(warn=-1)
