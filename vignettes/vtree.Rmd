---
title: "Introduction to vtree 1.2.4"
...
A tree with two variables is essentially the same as a two-way contingency table.
...
see [Variable Specification](#VariableSpecification}
...
## Variable specifications {#VariableSpecification}
...
Additionally, there are several special ways to specify variables.
 
 
### `is.na:` &mdash; Replacing a variable with a missing value indicator
...
### `stem:` &mdash; Variable stems for REDCap checkboxes
...
### `variable=value` &mdash; Dichotomizing based on equaling a specific value
...
### `variable<value`, `variable>value`  &mdash; Dichotomizing based on inequality  
...
For example, the `esoph` data set
(data from a case-control study of esophageal cancer in Ille-et-Vilaine, France, one of the R datasets)
...
Another example, based on the `HairEyeColor` data set
(one of the R datasets).
That data set is an array representing a contingency table.
...
There are a lot of combinations but let's say are especially interested
...
```{r, eval=FALSE}
vtree(titanic,"Class=Crew Sex Age Survived")
```
