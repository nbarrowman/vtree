---
title: "Introduction to vtree 4.3.7"
...
*Among African countries with a population over 30 million, what percentage are landlocked?* The variable tree below answers this question:
...
The diagram above was produced by vtree.
Both the structure of the diagram and the numbers shown were automatically determined.
When manual calculation and transcription are instead used
to populate such diagrams, mistakes are likely.
And although the errors that make it into to published articles are often small,
...
For example, the code below generates single-line labels such as
"Moderate, 16 (40%)":
...
Specifying `Ind*` matches all variable names that start with `Ind`
and end with any other characters.
In `FakeData` this matches `Ind1`, `Ind2`, and `Ind3`,
But in general `Ind*` would also match `India` and `India1977`.
 

### prefix `any:` with suffix `*` or `#`

A specification like `any:Start#` generates a variable representing whether any of the
matching variables (`Start1`, `Start2`, ...) equals 1 (or is TRUE).
Similary, `any:Start*` would do the same thing but a broader class of variable names
would be matched.


### prefixes `stem:`, `rc:` and `any:` (with no prefix)
...
If we wish to display these summaries for `Score` in each node of a variable tree,
say based on `Severity`, we simply call:
...
Variables in the `summary` parameter can also be specified in a way that is
somewhat similar to the [specification of variables](#VariableSpecification)
...
we specify `Category=single` in the `summary` argument and use the `%pct%` code:
...
As with variable specifications, there is a `stem:` prefix for variables
in the summary argument. 
This is used to identify checkbox variables from a REDCap dataset.
