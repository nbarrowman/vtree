---
title: "Introduction to vtree 4.4.1"
...
The two main uses of vtree are:

1. to explore a data set interactively

2. to produce customized figures for reports and publications.

Note that vtree is *not* designed to build or display decision trees.

The sections that follow introduce variable trees and provide an
...
*Among African countries with a population over 30 million, what percentage are landlocked?* The variable tree below shows the answer:
...
But it's often even more tedious, for two reasons. 
(1) If there are missing values it's harder to determine denominators.
...
When manual calculation and transcription are instead used
to populate such diagrams, mistakes are likely.
And although the errors that make it into published articles are often minor,
they can sometimes be disastrous.
One motivation for developing vtree was to make flow diagrams reproducible.
...
Although many of the examples here involve just two variables,
...
at each level of a variable tree,
all of the nodes correspond to values of a single variable.
...
although the variable tree may seem more intuitive.
...
Higher-order contingency tables become increasingly difficult to interpret.
Variable trees, on the other hand,
have the same intuitive structure regardless of the number of variables.
...
If `vtree` is called without a list of variables,
only the root node is shown, representing the entire data frame.
Although a tree with just one node might not seem very useful,
it provides a convenient way of displaying
...
Next, consider a variable tree with two variables, `Severity` and `Sex`.
To make it vertical, specify `horiz=FALSE`.
...
Each legend node also contains an overall (*marginal*) count,
...
the marginal counts and percentages for `Severity` shown in the legend nodes are identical to those displayed in the nodes of the variable tree.
...
and in each node put the node label on the same line as the count and percentage.

Consider this call to vtree:

```{r,eval=FALSE}
vtree(FakeData,"Severity")
```

The label for the Moderate node, for example,
is on two lines: "Moderate" and "16 (40%)".
Specifying `sameline=TRUE` results in single-line labels:

```{r,eval=FALSE}
vtree(FakeData,"Severity",sameline=TRUE)
```

The label for the Moderate node is "Moderate, 16 (40%)".
...
It is important to note how the `keep` parameter works
...
of the `r sum(!is.na(FakeData$Severity))` non-missing cases.)

On the right, the NA node is not shown, 
...
For example in the variable trees above,
not all of the severity groups are shown,
so their counts do not add up to `r nrow(FakeData)`,
...
Additionally, several modifiers can be used in the string of variable names.
...
and end with one or more numeric digits, namely `Ind1`, `Ind2`, and `Ind3`.
...
In `FakeData` this matches `Ind1`, `Ind2`, and `Ind3` (just like `Ind#` does).
But if `FakeData` contained variables named `Ind` and `Index,
they would also be matched by `Ind*`.


### prefix `any:` with suffix `*` or `#`

Sometimes a group of variables contain responses to 
a list of checkbox options ("check all that apply").
It is often useful to know if *any* of the options were selected (i.e. checked).

A specification like `any:Start#`
is used to generate a variable that is
`TRUE` if any of the matching variables has a "checked" value, or
`FALSE` if all of the matching variables have "unchecked" values.
By default, the parameters `checked` and `unchecked` specify which values
are considered checked or unchecked respectively, and are as follows:

|**parameter** | **Values**
|:-------------|-----------------------------------------------------------
|`checked`     | `"1"`,`"TRUE"`,`"Yes"`,`"yes"`
|`unchecked`   | `"0"`,`"FALSE"`,`"No"`,`"no"`

Values not listed in `checked` or `unchecked` are treated as missing values.


### prefixes for REDCap datasets: `stem:`, `rc:` and `any:` (without suffix `*` or `#`)

In datasets exported from [REDCap](https://www.project-redcap.org/),
checkboxes are represented using multiple variables.
Special prefixes make it easier to work with them.
...
The `summary` parameter allows you to show summary information about
a specified variable.

### Default summaries

#### Default summaries for numeric variables

For example, suppose you are interested in summary information for the `Score`
variable for all of the observations in the data frame (i.e. the root node).
...
Suppose we're building a variable tree based on `Severity` and we
wish to display these summaries for `Score` in each node.
We simply call:
...
#### Default summaries for non-numeric variables

If we want summary information for a factor variable such as `Category`,
a similar call to `vtree` can be used:

```{r,eval=FALSE}
vtree(FakeData,summary="Category")
```

For non-numeric variables like factors, the count and percentage for each value will be shown.
...
```{r,eval=FALSE}
vtree(FakeData,"Severity",summary="Category=single",sameline=TRUE,horiz=FALSE)
```

`r vtree(FakeData,"Severity",summary="Category=single", 
  horiz=FALSE,sameline=TRUE,
  width=450,height=150,pxwidth=800,imageheight="1in")`
...
`r vtree(FakeData,"Severity",summary="Ind*",sameline=TRUE,horiz=FALSE,just="l",
  width=450,height=150,pxwidth=800,imageheight="2in")`
  
Note that `just="l"` specifies that all text should be left-justified,
which conveniently lines up all of the rows of the summary.


##### prefixes for REDCap datasets: `stem:` and `stemc:`

For REDCap checklist variables,
there is a `stem:` prefix like the one for variable specifications.
There is an additional prefix, `stemc:`,
which calculates response patterns (combinations).
This is described in the section on [REDCap checkboxes](#REDCapCheckboxes)
later in this vignette.
...
The mean score is shown in each node.
If there are any missing values in a node,
the number of missing values is shown using the abbreviation `mv`.
The specification used above, `summary="Score \nmean score: %mean%"`,
has a special structure:

* First, the name of the variable for which a summary is desired. (We'll see later that variable specifications and expressions can also be used, as long as they do not contain any spaces.)

* Next a space.

* The remainder of the string specifies what to display,
with text as well as special codes (such as `%mean%`) to indicate the type of summary desired and to control which nodes display the summary, etc.

For example specifying `summary="Score \nmean score: %mean%"`
indicates that the variable to be summarized is `Score` and that the summary
should have the following format: `\nmean score: %mean%`.
This is rendered as:

* a new line (specified by the newline character `\n`) 

* followed by the words "mean score: "

* and finally the mean of `Score`.

If you don't want to see the number of missing values, 
the code `%meanx%` can be used instead of `%mean%`
(but note that this can be misleading).
...
The following codes control this:
...
-  -------  ------  --  -------  ------  --  -------  -------  ------  ------ 
1  Reds     &nbsp;   5  Purples  &nbsp;  9    YlOrBr  &nbsp;   13      RdYlGn  
2  Blues    &nbsp;   6  YlGn     &nbsp;  10   PuBuGn  &nbsp;   14      Set1   `
3  Greens   &nbsp;   7  PuBu     &nbsp;  11   BuPu    &nbsp;   &nbsp;  &nbsp;  
4  Oranges  &nbsp;   8  PuRd     &nbsp;  12   YlOrRd  &nbsp;   &nbsp;  &nbsp;  
-  -------  ------  --  -------  ------  --  -------  -------  ------  ------
...
To examine whether *any* of the checkboxes in a group have been checked,
the `any:` prefix can be used. 

```{r eval=FALSE}
vtree(dessert,"any:IceCream")
```

The `any:` stem reduces the checkbox variables to a single variable
representing whether one or more of the boxes have been checked (`TRUE`)
or none of them have been checked (`FALSE`).

### The `stem:` and `stemc:` prefixes in `summary`

The `summary` parameter supports a `stem:` prefix:
...
`r vtree(dessert,summary="stemc:IceCream",splitwidth=Inf,just="l",
  width=450,height=150,pxwidth=1000,imageheight="1.2in")`
