---
title: "Introduction to vtree 2.4.0"
...
`vtree` produces these diagrams and labels them with counts, percentages,
and other summaries.
`vtree` can be used:

* to explore a data set interactively, and

* to produce customized figures for reports and publications.
...
In many situations the case becomes more tedious, for two reasons.
...
When performed by hand, nested subsetting is taxing and error prone.

Nested subsets arise in all kinds of situations.
Consider, for example, flow diagrams for clinical studies,
such as the following rudimentary [CONSORT](http://www.consort-statement.org) diagram.
...
but the variable tree may be easier for some people to interpret.
...
Thus one use of variable trees is as an alternative to contingency tables.
...
* support for [checkbox variables](#REDCapCheckboxes) from [REDCap](https://www.project-redcap.org) databases; and
...
`r vtree(FakeData,"Severity Sex",horiz=FALSE,plain=TRUE,
  width=450,height=200,pxwidth=1600,imageheight="1.9in")`
...
`r vtree(FakeData,"Severity Sex",showlegend=TRUE,shownodelabels=FALSE,
  width=550,height=450,pxwidth=1200,imageheight="4in")`
...
*Pruning* a tree means removing certain nodes along with their descendants.
...
The `prunebelow` parameter does this,
and uses the same type of argument as the `prune` parameter. 
...
rather than those that should be discarded.
The `keep` parameter is used for this purpose,
and can thus be considered the opposite of the `prune` parameter.

The opposite of the `prunebelow` function is the `follow` function.
The descendants of nodes that are followed will be included in the tree,
whereas the descendants of the other nodes will be pruned.
...
`r vtree(FakeData,"Severity Sex Age Category",sameline=TRUE,imageheight="6in")`
...
By default, `prunesmaller` does not prune missing value nodes,
because percentages can be confusing when missing values are not shown.
You can override this behavior
(i.e. permit `prunesmaller` to prune missing value nodes)
by specifying `prunesmallerNA=TRUE`.
...
`r vtree(FakeData,"Group Sex",horiz=FALSE,
  ...
  tlabelnode=list(
    ...
    c(Group="B",Sex="F",label="woman"),
    c(Group="B",Sex="M",label="man")),
  width=700,height=250,pxwidth=1000,imageheight="2.2in")`
...
For convenience,
`vtree` allows you to specify the variable names (separated by whitespace)
...
* The remainder of the string specifies what to display, with special codes to indicate the type of summary desired and to control which nodes display the summary, etc.
...
If this new pattern variable is used as the first variable in a tree,
...
in order to simplify the display the root node is not shown.
(Admittedly, lacking a root node, the result is no longer technically a tree.) 
...
That being said, in some cases the variables have a natural ordering
...
A pattern tree looks a lot like a table.
...
and a column for percentage.
Compared to a variable tree, this table is much more compact,
and may be more suitable for use in a manuscript.

### Indicator variables

Pattern trees are useful for variables take values 0/1, no/yes, FALSE/TRUE, etc.,
i.e. *indicator variables*.
For convenience we'll refer to 0 (or no, FALSE, etc.) a *negative*
and 1 (or yes, TRUE, etc.) as an *affirmative*.
For example, the variables `Ind1` through `Ind4` in `FakeData` are 0/1 variables.
...
* the total sample size (`r nrow(FakeData)`) and percentage (100).

* the total number (N) of affirmatives for each variable, together with a percentage.

The `VennTable` function can also be used in R Markdown.
...
Note that the rows `n` and `pct` represent the frequency and percentage
of the total number of cases for each pattern of missingness,
and the columns `N` and `pct` at the bottom represent the frequency and percentage
of missingness for each variable.

It may be useful to identify the ids for these patterns.
...
it can can be pasted into an online Graphviz editors,
for example:
...
Note that `vtree` uses the `DiagrammeR` package
(which in turn is built on the open source graph visualization software,  `Graphviz`).
DiagrammeR (and hence `vtree`) automatically generates an htmwidget for display in HTML,
...
However, by default, when `vtree` is called from R Markdown,
it instead automatically converts the htmlwidget to a PNG file.

### Generating PNG files

PNG files are useful because they allow you to display variable trees in Microsoft Word documents,
and also because HTML files that use `htmlwidgets` can be large,
...
which is an adequate resolution for ordinary usage.
...
an htmlwidget can be used rather than embedding a PNG file.
(In fact PNG files are generated from htmlwidgets via SVG format.)
...
````
`r ''````{r}
cat(vtree(FakeData,"Severity Sex",pngknit=FALSE))
```
````
...
`r vtree(FakeRCT,"eligible randomized group followup analyzed",plain=TRUE,
  keep=list(eligible="Eligible",randomized="Randomized",followup="Followed up"),
  horiz=FALSE,showvarnames=FALSE,title="Assessed for eligibility",
  width=230,height=500,pxwidth=500,imageheight="6in")`
...
`r vtree(FakeRCT,"eligible randomized group followup analyzed",plain=TRUE,
  follow=list(eligible="Eligible",randomized="Randomized",followup="Followed up"),
  horiz=FALSE,showvarnames=FALSE,title="Assessed for eligibility",
  width=400,height=500,pxwidth=800,imageheight="6in")`
...
Since IDs are not as useful in the root note,
...
# Generating PNG files directly {#GeneratingPNGdirectly}
