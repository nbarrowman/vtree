---
title: "Introduction to vtree 1.0.4"
...
### Showing summaries depending on the contents of each node using the `runsummary` parameter
...
(Feature 1 and Feature 2) can be recorded.
Let's make up variables `FeatureI` and `FeatureII` but set them to NA if the `Viral` variable is FALSE or NA.

```{r}
set.seed(1234)
FakeData$FeatureI <- rbinom(46,1,0.5)
FakeData$FeatureI[!FakeData$Viral | is.na(FakeData$Viral)] <- NA
FakeData$FeatureII <- rbinom(46,1,0.5)
FakeData$FeatureII[!FakeData$Viral | is.na(FakeData$Viral)] <- NA
```
...
```{r,eval=FALSE}
vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=50,
  summary=c(
    "Viral viral: %npct% %leafonly%",
    "FeatureI \nFeature 1: %npct% %leafonly%",
    "FeatureII \nFeature 2: %npct% %leafonly%"))
```
```{r,echo=FALSE}
options(warn=-1)
v13a <- vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=50,width=950,height=900,
  summary=c(
    "Viral viral: %npct% %leafonly%",
    "FeatureI \nFeature 1: %npct% %leafonly%",
    "FeatureII \nFeature 2: %npct% %leafonly%"))
...
so it is not meaningful to describe the features of the viruses&mdash;since there aren't any.
...
```{r,echo=FALSE}
options(warn=-1)
v13b <- vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=50,width=950,height=900,
  summary=c(
    "Viral viral: %npct% %leafonly%",
    "FeatureI \nFeature 1: %npct% %leafonly%",
    "FeatureII \nFeature 2: %npct% %leafonly%"),
