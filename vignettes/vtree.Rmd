---
title: "Introduction to vtree 2.3.1"
...
Two factors make the task more tedious.
...
The leaf nodes represent all the observed combinations of *all* of the variables.
...
then the branches of the tree will be greatly simplified:
...
the root node is not shown in order to further simplify the display
...
Instead of showing a pattern tree, it may be more convenient to produce a table.
...
Compared to a variable tree, this table representation is much more compact,
and may be more suitable for a manuscript.

### Indicator variables

Pattern trees are useful for indicator variables
(such as variables that take values 0/1, no/yes, FALSE/TRUE, etc.).
...
around each element, it is preferable to call `print` and specify `quote=FALSE`:
...
with checkmarks and other features.
...
Summaries can be useful in pattern tables.
...
`vtree` has a number of special features for use with R Markdown.
