---
title: "Introduction to vtree 1.3.2"
...
* `%IQR%` IQR, i.e. p25, p75
* `%npct%` frequency and percentage of a logical variable.
By default "valid percentages" are used.
Any missing values are also reported.
* `%npct=Value%` frequency and percentage equal to Value.
* `%pct%` same as `%npct%` but percentage only (with no parentheses).
* `%pct=Value%` percentage equal to Value.
...
Now consider the `Category` variable,
and suppose we want to know the proportion that are in the "single" category.
The `%npct%` code can also be specified as `%npct=single%`:

```{r,eval=FALSE}
vtree(FakeData,"Severity",summary="Category \n%npct=single% single",horiz=FALSE,
  showvarnames=FALSE,sameline=TRUE)
```
...
Like `%npct%`, the `%pct%` can also be specified as `%pct=Value%`
where Value is a value of the summary variable.
