---
title: "Introduction to vtree 1.4.2"
...
Each leaf node in a tree provides the frequency of a particular combination values
of the variables.
By specifying `pattern=TRUE`, an alternative display is produced:
...
Also note that when `pattern=TRUE`, percentages are also not shown by default.
...
In fact the corresponding table can be exported by specifying `ptable=TRUE`:

```{r}
vtree(FakeData,"Severity Sex",pattern=TRUE,ptable=TRUE)
```

This compact representation may be more convenient for display in a manuscript.
...
since only the full *n*-way intersections (when there are *n* variables)
are represented:
...
Specifying `ptable=TRUE` produces this information in table format:

```{r}
vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,ptable=TRUE)
```

It is particulary useful to combine `ptable=TRUE` with a `summary`.
For example, we might wish to examine the ids in each of the combinations:

```{r}
vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,summary="id %list%%trunc=10%",ptable=TRUE)
```
