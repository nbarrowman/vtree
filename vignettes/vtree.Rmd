---
title: "Introduction to vtree 2.0.8"
...
cat(vtree(FakeData,"Severity Sex",showlegend=FALSE,horiz=FALSE,
  width=600,height=200,pxwidth=800,imageheight="2.2in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity",
  width=250,height=250,pxwidth=1000,imageheight="2.5in",folder=PNGdir))
...
```
```{r, echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Sex",horiz=FALSE,plain=TRUE,
  width=550,height=200,pxwidth=800,imageheight="1.6in"))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Sex",showlegend=TRUE,shownodelabels=FALSE,
  width=550,height=450,pxwidth=800,imageheight="4in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Sex",prune=list(Severity=c("Mild","Moderate")),
  width=500,height=300,pxwidth=800,imageheight="2.5in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Sex",prunebelow=list(Severity=c("Mild","Moderate")),
  width=500,height=300,pxwidth=800,imageheight="3in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Sex",horiz=FALSE,
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Group Sex",horiz=FALSE,labelnode=list(Sex=c(Male="M",Female="F")),
  width=700,height=250,pxwidth=800,imageheight="2.2in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Group Sex",horiz=FALSE,
  labelnode=list(Group=c(Child="A",Adult="B")),
  tlabelnode=list(
    c(Group="A",Sex="F",label="girl"),
    c(Group="A",Sex="M",label="boy"),
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Group Severity",horiz=FALSE,showvarnames=FALSE,
  text=list(Severity=c(Mild="\n*Excluding\nnew diagnoses*")),
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity",summary="Score \nmean score: %mean%",
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity",horiz=FALSE,showvarnames=FALSE,splitwidth=Inf,sameline=TRUE,
  summary=c(
    "Score \nScore: mean (SD) %mean% (%SD%)",
    "Pre \nPre: range %min%, %max%"),
  width=650,height=350,pxwidth=1000,imageheight="1.7in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity",summary="Viral \nViral %npct%",horiz=FALSE,
  showvarnames=FALSE,sameline=TRUE,
  width=650,height=200,pxwidth=800,imageheight="1.5in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity",summary="Category=single \n%pct% single", 
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Sex Category",sameline=TRUE,splitwidth=150,varminwidth=c(Category=6),
  summary=c(
    "Viral \nviral: %npct%%leafonly%",
    "Feature1 , F1: %npct%%leafonly%",
    "Feature2 , F2: %npct%%leafonly%"),
  pxwidth=1000,imageheight="2.5in",folder=PNGdir))
...
cat(vtree(FakeData,"Severity Sex",
  width=650,height=650,pxwidth=1000,imageheight="4in",folder=PNGdir))
...
```
```{r, echo=FALSE,results="asis"}
cat(vtree(FakeData,"Ind1 Ind2 Ind3 Ind4",Venn=TRUE,
  width=650,height=650,pxwidth=1000,imageheight="7in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Ind1 Ind2 Ind3 Ind4",Venn=TRUE,pattern=TRUE,
  palette=c(Ind1=1,Ind2=2,Ind3=3,Ind4=4),
  ,pxwidth=800,imageheight="5.5in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,
  width=700,height=370,pxwidth=800,imageheight="3.5in",folder=PNGdir))
...
```
```{r,echo=FALSE,results="asis"}
cat(vtree(FakeRCT,"eligible randomized group followup analyzed",plain=TRUE,
  keep=list(eligible="Eligible",randomized="Randomized",followup="Followed up"),
  horiz=FALSE,showvarnames=FALSE,title="Assessed for eligibility",
  width=230,height=500,pxwidth=800,imageheight="6in",folder=PNGdir))
...
cat(vtree(FakeRCT,"eligible randomized group followup analyzed",plain=TRUE,
  ...
  horiz=FALSE,showvarnames=FALSE,title="Assessed for eligibility",
  summary="id \nid: %list% %noroot%",
  width=500,height=600,pxwidth=800,imageheight="6in",folder=PNGdir))
...
# Using vtree interactively or from R Markdown
...
* If `vtree` is called while an R Markdown file is being knitted, it generates a PNG file and automatically embeds it into the knitted document. Successive PNG files are named `vtree1.png`, `vtree2.png`, and so forth and are stored in the current folder, or the folder specified by the `folder` parameter. During knitting, `vtree` uses a global counting variable called `vtcount` to keep track of the PNG files. If the `pngknit` parameter is set to `FALSE`, then PNG files are not generated or embedded.

## R Markdown

To use vtree in R Markdown, you can use inline code:
...
Code chunks have the advantage that they can also be run interactively (for example within RStudio, by clicking of the green arrow at the right-hand side of a code chunk).


# Advanced: Saving to PNG files 

When `vtree` is called within an R Markdown file that is being knitted,
it automatically generates and embeds PNG images,
with no further steps required.

If you prefer to generate those files yourself,
you can specify `pngknit=FALSE` and follow the steps described below.
...
## Advanced: The `grVizToPNG` function
...
## Advanced: Embedding a PNG image into R Markdown output
