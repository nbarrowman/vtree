---
title: "Introduction to vtree 1.2.8"
...
One example of nested subsetting is in flow diagrams for clinical studies,
which often involve 
*participants* who are a subset of
...
The errors that make it to publication are often small,
but occasionally they can be disastrous.
...
A tree with two variables can be compared to a two-way contingency table.
...
Similarly, a tree with `Severity` shown within levels of `Sex` corresponds to
a contingency table with *row percentages*.

The contingency table above is much more compact than the corresponding variable tree,
but some people find the variable tree easier to interpret.
When three of more variables are of interest,
multi-way contingency tables are typically displayed using several two-way tables.
In this situation, variable trees are much easier to interpret.

It is also noteworthy that contingency tables are not *always* more compact than variable trees.
When most cells of a large contingency table are empty (known as a *sparse* table),
...
For example, `vtree` makes it easy to display the mean (or other summary statistic)
of a quantitative variable such as age within each node.
Finally, `vtree` supports numerous additional customizations and formatting tweaks.

To summarize, `vtree` supports:

* flexible pruning to remove parts of the tree that are of lesser interest

* display of information about other variables in each node
(e.g. summary statistics for numeric variables)
...
see the section on [Variable specification](#VariableSpecification) below.
...
```{r,eval=FALSE}
vtree(FakeData,"Severity Sex",horiz=FALSE,labelvar=c(Severity="Severity on day 1"))
```
```{r,echo=FALSE}
...
For example, to display the mean of the numeric variable `Score`,
...
* `%pct%` same as `%npct%` but % only (with no parentheses).
...
It may also be convenient to only show the output from `%list` in certain nodes.
...
vtree(FakeData,"Severity",summary="id \nid = %list%%var=Severity%%trunc=40%",
  horiz=FALSE,showvarnames=FALSE,sameline=TRUE)
```
```{r,echo=FALSE}
v13 <- vtree(FakeData,"Severity",summary="id \nid = %list%%var=Severity%%trunc=40%",
  horiz=FALSE,showvarnames=FALSE,sameline=TRUE,width=650,height=200)
...
## Variable specification {#VariableSpecification}
...
Additionally, there are several modifiers that affect the way that variables are represented in a tree.
 
### prefix `is.na:` &mdash; Replacing a variable with a missing value indicator

Specifying individual variable names with the prefix `is.na:`
replaces them with a missing value indicator in the variable tree.
...
### prefix `stem:` &mdash; Variable stems for REDCap checkboxes
...
### prefix `tri:` &mdash; Trichotomizing variables to identify extreme values
...
**Note:** The `tri:` prefix is unique compared to other variable prefixes
in that it takes effect within each node,
rather than at the level of the entire data frame.

The `tri:` prefix trichotomizes a numeric variable within a node
based on the median and the IQR, both computed within the node.
The result is
a "mid" group representing values within plus or minus 1.5&times;IQR of the median,
a "hi" group more than 1.5&times;IQR above the median,
and a "lo" group more than 1.5&times;IQR below the median.


### specification `variable=value` &mdash; Dichotomizing based on equaling a specific value
...
### specification `variable<value`, `variable>value`  &mdash; Dichotomizing based on inequality  

These two specifications can be useful for identifying subsets with extreme values.
**Warning:** The specified value is treated as *numeric*, not as a character string.
...
Note that percentages are not shown by default.
Specifying `showpct=TRUE` displays percentages.
Let's also show the legend:


```{r, eval=FALSE}
vtree(FakeData,"Ind1 Ind2 Ind3 Ind4",Venn=TRUE,showpct=TRUE,showlegend=TRUE)
```
```{r, echo=FALSE}
...
```{r,eval=FALSE}
pattern <- with(FakeData,paste(Ind1,Ind2,Ind3,Ind4))
```
```{r,echo=FALSE}
pattern <- with(FakeData,paste(Ind1,Ind2,Ind3,Ind4,sep="|"))
```
Here are the observed frequencies of these patterns:

`Ind1` | `Ind2` | `Ind3` | `Ind4` | Frequency
---------|--------|--------|--------|----------------
`r paste(paste0(names(table(pattern)),"|",table(pattern)),collapse="\n")`

Note that these are the frequencies shown in the rightmost nodes of the
variable tree above.

Suppose `pattern` is put into the data frame.
If `vtree` is called and `pattern` is listed as the first variable,
then a special variable tree will be produced.
After the first level (representing `pattern`),
each subsequent node will, by definition, have only one child.
The resulting variable tree will thus separate the observed patterns and
the branches of the tree will be linear.

Instead of building the patterns yourself and then using them in a call to `vtree`,
these steps can be performed automatically.
Specifying `pattern=TRUE` directly produces a variable tree that separates the patterns:
...
For simplicity the nodes are not labeled, except for the `pattern` nodes.
...
To visualize the patterns of missing values, `pattern=TRUE` can be specified.


```{r,eval=FALSE}
vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,pattern=TRUE,
  palette=c(Severity=1,Age=2,Pre=3,Post=4))
```
```{r,echo=FALSE}
v25 <- vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,pattern=TRUE,
  palette=c(Severity=1,Age=2,Pre=3,Post=4))
...
Let's use the following function (from the R Cookbook site, http://www.cookbook-r.com/Manipulating_data/Converting_between_data_frames_and_contingency_tables/#cases-to-contingency-table) to convert the array to a dataframe:
...
```{r, eval=FALSE}
hec <- countsToCases(as.data.frame(HairEyeColor))
```
...
```{r, eval=FALSE}
vtree(titanic,"Class Sex Age Survived",sameline=TRUE)
```
