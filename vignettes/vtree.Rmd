---
title: "Introduction to vtree 2.2.9"
...
(0 = member, 1 = non-member), then a pattern tree is an alternative representation of a Venn diagram.
There are convenient settings in `vtree` for indicator variables

Convenient settings for such variables can be requested by specifying `Venn=TRUE`:
...
A pattern table for indicator variables provides all the information that a Venn diagram represents.
For example:
...
To generate a table including markdown codes for checkmarks and other features,
specify `markdown=TRUE`.
...
Except for the pattern node, all the other nodes are blank,
...
```{r}
print(VennTable(vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,ptable=TRUE)),
  quote=FALSE)
```
