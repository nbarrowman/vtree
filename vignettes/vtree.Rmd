---
title: "Introduction to vtree 2.3.4"
...
with variables named `population`, `continent`, `landlocked`, etc.
...
The task often becomes more tedious, for two reasons.
...
Mistakes in flow diagrams are all too common in published studies 
because manual calculation and transcription are so error-prone.
And although the errors that make it to publication are often small,
they can sometimes be disastrous.

`vtree` is a solution to the problem of calculating nested subsets
...
Variable trees thus provide an alternative to multi-way contingency tables
...
* study-participant flow diagrams
...
These combinations, or "patterns", can be considered as an additional variable.
...
(Technically, lacking a root node, the result is no longer a tree.) 
...
which converts the pattern table to a matrix of character string
...
Since a matrix of character strings is ordinarily displayed with quotation marks
around each element, the result is ... unattractive.
Instead it's helpful to call `print` and specify `quote=FALSE`:
...
where a checkmark represents affirmative, a space representing negative,
and a dash represents missing
(which can be changed with the `NAcode` parameter).
Note that when `markdown=TRUE`,
...
Note that the columns `n` and `pct` represent the frequency and percentage
of the total number of cases for each pattern of missingness,
and the rows `N` and `pct` at the bottom represent the frequency and percentage
