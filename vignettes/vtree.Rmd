---
title: "Introduction to vtree 2.4.5"
...
For example, to retain patients whose `Severity` is Moderate:
```{r,eval=FALSE}
...
(i.e. excluding the number of missing values from the denominator,
which is the default)
and on the right using percentages that include the number of missing values in the denominator.
...
`r vtree(FakeData,"Severity",imagewidth="1.5in")`
`r vtree(FakeData,"Severity",vp=FALSE,imagewidth="1.5in")`

Suppose we use `keep` to retain only Moderate cases.
...
`r vtree(FakeData,"Severity",keep=list(Severity="Moderate"),imagewidth="1.5in")`
`r vtree(FakeData,"Severity",vp=FALSE,keep=list(Severity="Moderate"),imagewidth="1.5in")`
...
This is done because the denominator being used here is `r nrow(FakeData)`,
regardless of the number of missing values.
...
`r vtree(FakeData,"Severity Sex Age Category",sameline=TRUE,imageheight="7in")`
...
For convenience,
`vtree` allows you to specify the names of the variables of interest (separated by whitespace)
...
which automatically generates an htmlwidget for display in HTML,
using the [htmlwidgets](https://www.htmlwidgets.org/) framework.
Then `vtree` converts the htmlwidget into a PNG. 
...
During knitting, `vtree` uses the `options` function to keep track of
a counting variable called `vtcount` to count the PNG files,
and a folder variable called `vtfolder` to identify where to store them.
...
(for example within RStudio, by clicking on the green arrow at the right-hand side of a code chunk).
...
Note the differences between this and the code chunk to produce a PNG file
(in the previous section):
(1) you specify `pngknit=FALSE`,
(2) you don't specify the chunk option `results="asis"`,
and (3) you don't put `cat` around the call to `vtree`.
