---
title: "Introduction to vtree 1.1.3"
...
Suppose we have a data set of countries,
...
When there are missing values in the data set, it is even harder.
And as additional subsets are examined, the magnitude of the task grows rapidly.

A natural way to represent such nested subsets of a data set is to use a tree structure,
...
Variable trees are easy to interpret because they represent the structure of subsets visually, 
whereas contingency tables can be less intuitive, especially when they involve more than two variables.
...
Variable trees can be used for exploration of a data set,
for constructing figures to include in reports and publications, 
and also for double checking code to ensure that subsetting is performed correctly.
...
For example, vtree makes it easy to display the mean age within each node.
...
It can be difficult to clearly display a large variable tree.
...
each subsequent node will have only one child.
...
Specifying `pattern=TRUE` directly produces a vtree that separates the patterns:
...
This is done because, without percentages, in general arrows have no particular significance.
In some cases, however, there is a natural ordering to the variables
(as is the case with longitudinal variables).
In such cases, `seq=TRUE` can be specified instead of `pattern=TRUE`,
and a "sequence" will be shown, including arrows.
