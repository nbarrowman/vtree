---
title: "Introduction to vtree 2.3.9"
...
*Pruning* a tree means removing nodes along with their descendants.
...
As the number of nodes in a variable tree gets very large it becomes difficult, 
if you'll pardon the pun, to see the forest for the tree.
...
`r vtree(FakeData,"Severity Sex Age Category",sameline=TRUE,imageheight="5in")`

One solution is to prune nodes that contain small numbers of observations.
As with other types of pruning, the result is an incomplete tree,
but one that may be more helpful.
...
By default, `prunesmaller` will not prune missing value nodes,
because percentages can be confusing when missing values are not shown.
You can override this behavior
(to permit `prunesmaller` to prune missing value nodes)
