---
title: "Introduction to vtree 4.1.5"
...
One thing that vtree does *not* do, however, is build decision trees.

The sections that follow introduce the basic design and features of vtree.
Or you can [skip this and begin using the `vtree` function](#vtreeFunction).
...
But it's often even more tedious&mdash;for two reasons. 
...
Manual calculation and transcription are both error-prone tasks,
which can lead to mistakes in published flow diagrams.
And although the errors that make it to publication are often small,
they can sometimes be disastrous.
Part of the reason `vtree` was developed was to make flow diagrams reproducible.

Note that at the end of this vignette, there is a collection of
...
Next to each variable are "legend nodes" representing the values of that variable
...
from what is shown in the nodes because the nodes for `Sex` are nested within levels of `Severity`.
...
so it may be useful to specify, say, `prunesmaller=3`,
which prunes patterns than occur 2 times or fewer.
...
Other parameters for specifying colors include:

* `NAfillcolor`: By default, missing value nodes are colored white.
For a different color (say gray), specify `NAfillcolor="gray"`.
To instead use a color from the current palette, specify `NAfillcolor=NULL`.

* `rootfillcolor`: The color of the root node can be changed (say to yellow)
by specifying `rootfillcolor="yellow"`.

* `fillcolor`: To set all nodes of the tree
(except for missing value nodes and the root node) 
to be the same color (say palegreen),
specify `fillcolor="palegreen"`.

* `plain`: A simple color scheme is produced by specifying `plain=TRUE`.
(Additionally, this increases the spaces between nodes.)
