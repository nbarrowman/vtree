---
title: "Introduction to vtree 4.5.6"
...
and can be combined with othe prefixes.
...
will match both `openSaturday` and `openSunday`.
If we want the tree to branch according to which shops are open Saturday and Sunday, we can use
...
### Deprecated: prefixes for REDCap datasets: `stem:` and `rc:`
...
The `summary` argument can also use the prefixes `i:`, `any:`, `all:`, and `r:` as well as the suffixes `#` and `*`.



##### Deprecated: prefix for REDCap datasets: `stem:`
...
The table below lists numerous other summary codes.
When such a code is present, the default summary is not shown.
Instead,
any text that is provided (in this case `\nmean score\n`)
is shown, together with the requested summary information.
...
(and similarly `%medianx%` instead of `%median%`, etc.).
But note that results may be misleading if missing values are suppressed.
...
Specifying `getinfo=TRUE` results in a list being returned by `vtree`.
Here is an example:

```{r,echo=TRUE}
vtree(FakeData,"Group",getinfo=TRUE)
```

To understand the structure, it may be helpful to try a lengthier example
(output not shown):

```{r,echo=TRUE, eval=FALSE}
vtree(FakeData,"Group Viral",getinfo=TRUE)
```

The list contains the counts (`.n`) and percentages (`.pct`)
that appear in the tree.
It also contains vectors specifying the subset of the data frame
being used (`.subset`).

For example, to examine the subset of the data corresponding to
the `A` value of `Group` and the `NA` value of `Viral`,
the following commands can be used (output not shown):

```{r, eval=FALSE}
info <- vtree(FakeData,"Group Viral",getinfo=TRUE)
FakeData[info$Group$A$Viral$`NA`$.subset,]
```

This might seem like a strange way to subset a data frame,
but it means that the same code that draws a variable tree
can be used to extract subsets.
