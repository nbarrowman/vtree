---
title: "Introduction to vtree 1.1.5"
...
with variables representing population, continent, GDP, type of government, etc.
Suppose we wish to compare countries based on *continent*,
for example the countries in Africa with the rest of the countries.
Within each of these subsets,
we might wish to examine *nested* subsets based on *population*,
for example, countries with populations under 10 million, between 10 and 30 million,
and over 30 million.
We might continue to a 3rd level of nesting,
by considering countries that are landlocked or not.
...
Even in a simple situations like this,
it can be a chore to keep track of nested subsets and calculate percentages.
As the number of nested subsets increases,
the magnitude of the task grows rapidly.
To make matters worse, if there are missing values in the data set,
the task becomes even more complicated.
For these reasons,
it is best to avoid manual calculation of nested subsets of a data set.
To see where this can go wrong, consider study flow diagrams,
which involve nested subsets such as
*participants* within *eligible patients* within *patients who were assessed*.
Because mistakes inevitably arise during manual calculation and transcription,
it is not uncommon for published study flow diagrams to contain errors.
The errors that make it to publication are often small,
but occasionally they can be disastrous.

A solution to the problem of calculating and displaying nested subsets is presented here.
A natural way to represent nested subsets of a data set is to use a tree structure,
which we call a *variable tree*.
...
A tree with two variables is equivalent to a two-way contingency table.
...
but the variable tree is easier to interpret.
This is all the more so with multi-way contingency tables,
which are typically displayed using multiple two-way tables.
It is also noteworthy that contingency tables are not always compact.
When most cells of a contingency table are empty (i.e. the table is *sparse*),
the corresponding variable tree will seem comparatively compact,
since empty-nodes are not shown.
...
Variable trees can be used to explore a data set,
...
Note that `showroot=FALSE` has also been specified to illustrate how removing the root
node can simplify the display:

```{r,eval=FALSE}
vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,pattern=TRUE,showroot=FALSE,
  palette=c(Severity=1,Age=2,Pre=3,Post=4))
```
```{r,echo=FALSE}
v25 <- vtree(FakeData,"Severity Age Pre Post",check.is.na=TRUE,pattern=TRUE,showroot=FALSE,
  palette=c(Severity=1,Age=2,Pre=3,Post=4))
