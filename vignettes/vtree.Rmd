---
title: "Introduction to vtree 4.3.5"
...
The two main uses of vtree are to:
...
First, if there are missing values it's harder to determine denominators.
...
[summary information](#summary) about the data frame.
...
### Automatic variable tree

Specifying `auto=TRUE` generates a variable tree by automatically selecting
as many discrete variables as possible in the order they occur in the data frame.
This provides a quick demonstration of `vtree` but may not be especially
informative.
...
variable for all of the observations in the data frame.
In that case you don't need to specify any variables for the tree since you
are only interested in the root variable:
...
`r vtree(FakeData,summary="Score",
  width=450,height=150,pxwidth=1000,imageheight="1in")`
  
When a variable name (in this case `"Score"`) is specified as the argument
of the `summary` parameter, a default set of summary statistics is shown:
...
say based `Severity`, we simply call:
...
The next section explains this structure.

### How to specify the `summary` argument to get customized summaries

As described above, the `summary` argument may simply be a variable name
(in which case detailed summary statistics are reported).
Alternatively, a character string with the following structure may be used:
...
* a new line (represented by the newline character `\n`) 
...
To examine whether *any* of the checkboxes in a group have been
checked, the `any:` prefix can be used. 
