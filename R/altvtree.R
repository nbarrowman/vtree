  ...
  parent = 1, last = 1, root = TRUE)
{
  ...
  if (root) {
    ...
    if (!is.logical(splitspaces)) stop("splitspaces argument must be TRUE or FALSE")
    ...
    if (pattern) {
      if (!is.null(summary)) {
        colvars <- c("pattern","summary",vars[-1])
      } else {
        colvars <- vars
      }
      resultMatrix <- matrix("",nrow=length(PATTERN_levels),ncol=length(colvars))
      colnames(resultMatrix) <- colvars
      rownames(resultMatrix) <- PATTERN_levels
      for (var in vars[vars!="pattern"]) {
        for (pattern in PATTERN_levels) {
          resultMatrix[pattern,var] <- as.character(z[z$pattern==pattern,var][1])
        }
      }
      #print(resultMatrix)
    }
  ...
  if (pmat) {
    if (vars[1]=="pattern") {
      resultMatrix[,"pattern"] <- paste0(fc$value,fc$npctString)
      if (length(ThisLevelText)>0) {
        resultMatrix[,"summary"] <- ThisLevelText
      }
    } 
  }
